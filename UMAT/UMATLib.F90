...
!------------------------------------------------------------------------------
    ...
    E = Props(2)
    nu = Props(3)
    inquire(file="subout.txt",exist=exists)
    if(.not.exists) then 
      open(92,file="subout.txt",status="new")
      write(92,*) "NTENS = ",ntens
    end if 
    write(92,*) 'Called Subroutine Here'
    ...
    DO i=1,ntens
      write (92,*) "Stress for ",i,STRESS(i)," Element ",noel," IP ",NPT
      write (92,*) "Strain for ",i,STRAN(i)," Number Components ",NDI
      write (92,*) "Strain Increment ",DSTRAN(i)
