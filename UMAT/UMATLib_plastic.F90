...
!  ! List material constants as {density, Poisson's ratio,
!  ! strain 1, stress 1, strain 2, stress 2}
!  Material Constants(6) = Real 1.0 0.3 0.003 70000. 0.16 135000.
...
!------------------------------------------------------------------------------
    LOGICAL :: exists
    REAL(KIND=dp) :: strain1, yield1, strain2, yield2
    ...
    yield2 = Props(6)
      ...
      DO i=1,ntens
       ! E is calculated from the stress strain points
      if (stress(i).le.yield1) then
      E = (yield1/strain1)
      else
      E = ((yield2-yield1)/(strain2-strain1))
      end if
       LambdaLame = E * nu / ( (1.0d0+nu) * (1.0d0-2.0d0*nu) )
       MuLame = E / (2.0d0 * (1.0d0 + nu))
